...
export const Maintenance: React.FC = () => {
  ...
  } = useAsyncData<MaintenanceContract[]>(async () => {
    const rows = debouncedQuery
      ? await dataService.search<MaintenanceContract>('maintenance', debouncedQuery, [searchType])
      : await dataService.getAll<MaintenanceContract>('maintenance');
  ...
  const update = useAsyncAction(async () => {
    ...
    const updated = await dataService.update<MaintenanceContract>('maintenance', selectedContract.id, formData);
    await reload();
    setIsEditMode(false);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button
            onClick={() => { setFormData(createDefaultContract()); create.clearError(); setIsAddOpen(true); }}
      ...
      {/* ── Data ──────────────────────────────────────────────────────────── */}
      {isLoading ? (
        <LoadingState hint="يتم الآن جلب عقود الصيانة" />
      ) : error ? (
        <ErrorState error={error} onRetry={reload} isRetrying={isFetching} />
      ) : contracts.length === 0 ? (
        <EmptyState
          icon={Wrench}
          message={searchQuery ? 'لا توجد نتائج مطابقة لبحثك' : 'لا توجد عقود صيانة بعد'}
          hint={searchQuery ? 'جرّب كلمة بحث أخرى أو غيّر نوع البحث.' : 'اضغط «عقد جديد» لإضافة أول عقد صيانة.'}
        />
      ) : (
        <>
          {/* ── Desktop Table ─────────────────────────────────────────────── */}
