...
export const Workers: React.FC = () => {
  ...
  const loadWorkers = async () => {
    const data = await dataService.getAll<Worker>('workers');
  ...
  const handleSearch = async (query: string) => {
    ...
    if (!query) { loadWorkers(); return; }
    setWorkers(await dataService.search<Worker>('workers', query, [searchType]));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button
            onClick={() => { setFormData(createDefaultWorker()); setIsAddOpen(true); }}
