...
export const Sites: React.FC = () => {
  ...
  const loadSites = async () => {
    const data = await dataService.getAll<Site>('sites');
    setSites(data.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()));
  };

  const handleSearch = async (query: string) => {
    setSearchQuery(query);
    if (!query) { loadSites(); return; }
    setSites(await dataService.search<Site>('sites', query, [searchType]));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button
            onClick={() => { setFormData(createDefaultSite()); setIsAddOpen(true); }}
